...
class OptimizedTrainer:
    ...
    def __init__(self, config):
        ...
        self.optimizer = torch.optim.AdamW([
            {'params': backbone_params, 'lr': config['learning_rate'] * 0.1},  # Backbone 10배 느리게
            {'params': other_params, 'lr': config['learning_rate']}
        ], weight_decay=config['weight_decay'])
        
        print(f"\n📊 Differential Learning Rate:")
        print(f"  Backbone: {config['learning_rate'] * 0.1:.6f}")
        print(f"  Decoder:  {config['learning_rate']:.6f}\n")
    ...
    def train(self):
        ...
        for epoch in range(1, self.config['epochs'] + 1):
            ...
            if is_best:
                improvement = val_metrics['iou'] - self.best_val_iou
                self.best_val_iou = val_metrics['iou']
                self.patience_counter = 0
                print(f"  🎉 New best Val IoU: {self.best_val_iou:.4f} (+{improvement:.4f})")
            else:
...
def main():
    ...
    config = {
        ...
        'input_size': (384, 384),  # ✨ 해상도 증가! (320x320 → 384x384)
        ...
        # Training
        'batch_size': 4,  # FP16으로 4 안전
        'accumulation_steps': 3,  # 실질적 배치 = 4*3 = 12
    ...
    print(f"\n🚀 Key Improvements:")
    print(f"  ✨ Resolution: 320×320 → 384×384 (+44% pixels)")
    print(f"  ✨ Mixed Precision: FP16 (메모리 50% 절감)")
    print(f"  ✨ Differential LR: Backbone 10x slower")
    print(f"  ✨ Gradient Accumulation: Effective batch 12")
    print(f"  ✨ Epochs: 100 (2x longer)")
    print()
